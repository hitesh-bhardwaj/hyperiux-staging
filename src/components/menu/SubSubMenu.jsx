...
const SubSubMenu = ({ subSubMenu, activeSub }) => {
  ...
  const activeItems =
    activeSub === 1 ? menuItems : activeSub === 2 ? menuItems2 : [];

  const handleNestedRoute = (event, href) => {
      ...
      <div className="relative flex w-fit flex-col gap-[0.5vw]">
        ...
        {activeItems.map((label, index) => (
          <Link
            ...
            ref={(el) => {
              subSubItemRefs.current[index] = el;
            }}
            className="subsubmenu-reveal-row h-fit w-fit font-display"
            onClick={(event) => handleNestedRoute(event, "/solution-detail")}
